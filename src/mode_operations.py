...
from .core import Vec2, EPSILON, clamp_stick_value, clamp_stick_vec2, clamp_trigger_value
...
# =============================================================================
# GAMEPAD-SPECIFIC WRAPPERS (clamp outputs to valid ranges)
# =============================================================================

def apply_stick_mode(
    mode: str,
    canonical_value: Union[Vec2, float],
    accumulated: Vec2
) -> Vec2:
    """Apply mode to stick value and clamp result to [-1, 1] per axis.

    Args:
        mode: The mode (offset, override, scale)
        canonical_value: The canonical value
        accumulated: Current accumulated Vec2

    Returns:
        Clamped Vec2 in [-1, 1] range
    """
    result = apply_position_mode(mode, canonical_value, accumulated)
    return clamp_stick_vec2(result)


def apply_trigger_mode(
    mode: str,
    canonical_value: float,
    accumulated: float
) -> float:
    ...
    Args:
        mode: The mode (offset, override, scale)
        canonical_value: The canonical value
        accumulated: Current accumulated float

    Returns:
        Clamped float in [0, 1] range
    """
    result = apply_scalar_mode(mode, canonical_value, accumulated)
    return clamp_trigger_value(result)


def calculate_stick_target(
    operator: str,
    value: Union[tuple, Vec2],
    current: Vec2,
    mode: str
) -> Union[Vec2, float]:
    """Calculate target for stick property. Delegates to calculate_position_target."""
    return calculate_position_target(operator, value, current, mode)


def calculate_trigger_target(
    operator: str,
    value: float,
    current: float,
    mode: str
