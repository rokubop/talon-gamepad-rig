...
This file provides:
- Package version checking via gamepad_rig_version() action
"""
import json
...
def _get_version() -> tuple[int, int, int]:
    ...
    try:
        ...
        print(f"ERROR: talon-gamepad-rig failed to load version from manifest.json: {e}")
        return (0, 0, 0)

@mod.action_class
class Actions:
    def gamepad_rig_version() -> tuple[int, int, int]:
        """
        Returns the package version as (major, minor, patch).

        Usage: actions.user.gamepad_rig_version() >= (1, 2, 0)
        """
        return _get_version()
